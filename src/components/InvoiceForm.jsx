...
const InvoiceForm = (props) => {
  ...
  const dataPassed1 = props.location;
  ...
  const handleAddInvoice = () => {
    if (isEdit) {
      ...
      axios.post(`http://localhost:8080/api/v1/invoice/save/${dataPassed.clientId}`, formData).then((response) => {

    });
    // saving items to items table
    // axios.post(`http://localhost:8080/api/v1/item/save/${formData.invoiceNumber}`, formData.item).then((response) => {
    ...
    navigate(`/invoices/${dataPassed.clientId}`,{state:dataPassed})
  ...
  return (
    <Form onSubmit={openModal}>
      <div className="d-flex align-items-center">
        <BiArrowBack size={18} />
        <div 
        onClick={()=> navigate(`/invoices/${dataPassed.clientId}`,{state:dataPassed})}
        className="fw-bold mt-1 mx-2 cursor-pointer">
            <h5>Go Back</h5>
        </div>
      </div>
