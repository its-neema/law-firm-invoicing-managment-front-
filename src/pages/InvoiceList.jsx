...
const handleFetchInvoices = async () =>{
  await axios.get(`http://localhost:8080/api/v1/invoice/client/${dataPassed.clientId}`).then((response) => {
...
useEffect(() => {
  ...
  const handleCopyClick = () => {
    const invoice =1
    // getOneInvoice(copyId);
    if (!invoice) {
      alert("Please enter the valid invoice id.");
    } else {
      navigate(`/create/${copyId}`, {state:dataPassed});
    }
  };
  ...
  return (
    <Row>
      <Col className="mx-auto" xs={12} sm={12} md={12} lg={10}>
        <h3 className="fw-bold pb-2 pb-md-4 text-center">Firm Invoices</h3>
        <Card className="d-flex p-3 p-md-4 my-3 my-md-4 p-sm-3">
          {isListEmpty ? (
            <div className="d-flex flex-column align-items-center">
              <h3 className="fw-bold pb-2 pb-md-4">No invoices present</h3>
              {/* <Link to="/create"> */}
                <Button onClick={() =>navigate("/create",{state:dataPassed})} variant="primary">Create Invoice</Button>
              {/* </Link> */}
            </div>
          ) : (
            <div className="d-flex flex-column">
              <div className="d-flex flex-row align-items-center justify-content-between">
                ...
                {/* <Link to="/create"> */}
                  <Button onClick={() =>navigate("/create",{state:dataPassed})} variant="primary mb-2 mb-md-4">Create Invoice</Button>
                {/* </Link> */}
                <Button
